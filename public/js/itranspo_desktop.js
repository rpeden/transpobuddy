  ...
  Map = {
    ...
    createmap : function() {
        ...
        google.maps.event.addListener(this.map, 'dragend', function() {
            if (Map.map.zoom < 16) { return;} 
	    Map.redoBoundaries();
            Map.addpoints();
           });

	google.maps.event.addListener(this.map, 'zoom_changed', function() {
            if (Map.map.zoom < 16) { alert("Stops not available at this zoom level. Please zoom in to load more stops.");
				 return; }
    ...
    addpoints : function() {
       ...
       $.get("/stops?latMin=" + Map.Boundaries.swLat + "&latMax=" + Map.Boundaries.neLat + "&lngMin="
		...
		for(var key in result){
                    ...
                    var stopLatLng = new google.maps.LatLng(result[key]["stop_lat"] 
				     - .00001  ,result[key]["stop_lon"] + .0000050);
	 ...
	 this.Boundaries.swLat = this.map.getBounds().getSouthWest().lat();
	 this.Boundaries.swLng = this.map.getBounds().getSouthWest().lng();
	 this.Boundaries.neLat = this.map.getBounds().getNorthEast().lat();
	 this.Boundaries.neLng = this.map.getBounds().getNorthEast().lng();
	...
	return function() {
	    ...
	    $.get("/stop_times/" + stop_id, /*marker.position.lat() + "/" + marker.position.lng(),*/
				 function(response) {
				    ...
				    var table = []; 
				    table.push("<table class='table table-condensed'><tr><th colspan=1>Route</th><th>Destination</th><th colspan=1>Time</th></tr>");
				    for (var key in nextbuses){
				      ...
				      table.push("<tr><td>" + nextbuses[key]["route_short_name"] + "</td><td>" 
							 + nextbuses[key]["trip_headsign"]
							 + "</td><td>" + depTime + "</td></tr>");
				      		      
				    }
				    table.push('</table>');
				    $('#buses').html(table.join(''))
				    //$('#title').html('Buses');
				...
				//		$('#leftnav a').html("Home");
				//		$('#leftnav a').attr('href', 'index.html');
